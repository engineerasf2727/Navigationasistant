...
def read_direction_label():
    try:
        with open("next_direction.txt", "r") as file:
...
def lane_finding_pipeline(img, init, mtx, dist,straight_arrow_img, turn_right_arrow_img, turn_left_arrow_img):
    ...
    # OK YERLEŞTİRME BURADA GERÇEKLEŞECEK (Her kare için)
    direction_label = read_direction_label()
    centerline_x = (left_fitx + right_fitx) / 2
    arrow_h, arrow_w = straight_arrow_img.shape[:2]
    interval = int(arrow_h * 1.5)  # Ok boyutunun 1.5 katı aralıkla diz
    last_arrow_pos = None
    for i in range(len(ploty)-1, 0, -interval):
        pts = np.array([[[centerline_x[i], ploty[i]]]], dtype=np.float32)
        transformed_pts = cv2.perspectiveTransform(pts, M_inv)[0][0]

        pos_x = int(transformed_pts[0] - arrow_w / 2)
        pos_y = int(transformed_pts[1] - arrow_h / 2)

        # Oku her kare için overlay yap
        if pos_y >= 0 and pos_x >= 0 and (pos_y + arrow_h) < out_img.shape[0] and (pos_x + arrow_w) < out_img.shape[1]:
            out_img = overlay_transparent(out_img, straight_arrow_img, pos_x, pos_y)
            last_arrow_pos = (pos_x, pos_y)
    if last_arrow_pos:
        turn_arrow_img = None
        if direction_label == "turn_right":
            turn_arrow_img = turn_right_arrow_img
        elif direction_label == "turn_left":
            turn_arrow_img = turn_left_arrow_img
        if turn_arrow_img is not None:
            # Büyük dönüş okunu son düz okun biraz üzerine ekleyelim
            turn_arrow_h, turn_arrow_w = turn_arrow_img.shape[:2]
            pos_x = last_arrow_pos[0] - (turn_arrow_w // 2) + (arrow_w // 2)
            pos_y = last_arrow_pos[1] - turn_arrow_h - 20  # 20 px yukarıda olacak
            # Overlay işlemi:
            if 0 <= pos_y < out_img.shape[0] - turn_arrow_h and 0 <= pos_x < out_img.shape[1] - turn_arrow_w:
                out_img = overlay_transparent(out_img, turn_arrow_img, pos_x, pos_y)
...
def main():
    ...
    delay=int(1000 / fps)
    # Ok sadece bir kez yüklenir!
    straight_arrow_img = cv2.imread('arrow_transparent.png', cv2.IMREAD_UNCHANGED)
    turn_right_arrow_img = cv2.imread('right.png', cv2.IMREAD_UNCHANGED)
    turn_left_arrow_img = cv2.imread('left.png', cv2.IMREAD_UNCHANGED)

    # Ölçekle:
    straight_arrow_img = cv2.resize(straight_arrow_img, (80, 80))
    turn_right_arrow_img = cv2.resize(turn_right_arrow_img, (150, 150))
    turn_left_arrow_img = cv2.resize(turn_left_arrow_img, (150, 150))
    ...
    while True:
        ...
        img_out, angle, _, _ = lane_finding_pipeline(frame, init, mtx, dist, straight_arrow_img, turn_right_arrow_img, turn_left_arrow_img)
        init= False
        
        end_time = time.time()
        fps_val = 1.0 / (end_time - start_time)
        fps_values.append(fps_val)
        time_stamps.append(end_time - start_global)
        cv2.putText(img_out, f"FPS: {fps_val:.2f}", (1000, 50), cv2.FONT_HERSHEY_SIMPLEX,
